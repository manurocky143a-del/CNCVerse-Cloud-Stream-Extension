...
private fun String.hexToBase64UrlOrNull(): String? {
  ...
  private fun getDRMKeysFromLicenseServer(url: String, kid: String): String {
    ...
    // ── Subscription popup (once per app session if not subscribed) ───────────
...
class IptvPlaylistParser {
    ...
    fun parseM3U(input: InputStream): Playlist {
        ...
        while (i < allLines.size) {
            ...
            if (line.isNotEmpty()) {
                when {
                    line.startsWith(LivXowProvider.EXT_INF) -> {
                    ...
                    line.startsWith(LivXowProvider.EXT_VLC_OPT) -> {
    ...
    private fun String.isExtendedM3u(): Boolean =
        startsWith(LivXowProvider.EXT_M3U) || startsWith(LivXowProvider.EXT_INF) || startsWith("#KODIPROP")
